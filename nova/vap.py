...
class Encoder(nn.Module):
    ...
    def forward(self, obs_traj):
        ...
        obs_traj_embedding = self.spatial_embedding(obs_traj.reshape(-1, 2))  # (obs_len * batch, embedding_dim)
        obs_traj_embedding = obs_traj_embedding.view(
            -1, batch, self.embedding_dim
        )  # (obs_len, batch, embedding_dim)
        state_tuple = self.init_hidden(batch)
        output, state = self.encoder(obs_traj_embedding, state_tuple)
        final_h = state[0]  # (num_layers, batch, h_dim) 

        return final_h

class TrafficEncoder(nn.Module):
    """
    :input: traffic_state: Tensor with shape (seq_len, batch)
...
class VehicleEncoder(nn.Module):
    """
    :input: 
      - agent_type(type of vehicle): Tensor of shape (batch, 1)
      - size: Tensor of shape (batch, 2)
    :output: final_h: Tensor of shape (self.num_layers, batch, self.h_dim)
    """
    def __init__(self, agent_type_dim=6, embedding_dim=64, h_dim=64, mlp_dim=1024, num_layers=1, dropout=0.0):
        ...
        self.agent_type_embedding = nn.Embedding(agent_type_dim, embedding_dim)  # discrete data
        self.size_layer = nn.Linear(2, embedding_dim)  # contineous data
    ...
    def forward(self, agent_type, size):
        ...
        # shape of combined_embedding: (batch, 2 * embedding_dim)
...
class StateEncoder(nn.Module):
    ...
    def forward(self, state):
        batch = state.size(1)
        state_embedding = self.state_layer(state)  # shape: (batch, embedding_dim)
        state_tuple = self.init_hidden(batch)
...
class Decoder(nn.Module):
    ...
    def forward(self, last_pos, last_pos_rel, state_tuple, seq_start_end, vx,vy):
        ...
        decoder_input = decoder_input.view(1, batch, self.embedding_dim)  # (1, batch, embedding_dim)

        for _ in range(self.seq_len):
            output, state_tuple = self.decoder(decoder_input, state_tuple)  # (1, batch, h_dim)
            rel_pos = self.hidden2pos(output.view(-1, self.h_dim))  # (batch, 2)
            curr_pos = rel_pos + last_pos  # absoulte postion

            if self.pool_every_timestep:
                decoder_h = state_tuple[0]  # (1, batch, h_dim)
                pool_h = self.pool_net(decoder_h, seq_start_end, curr_pos, vx, vy)
                decoder_h = torch.cat(  # combine output of LSTM and pooling
                    [decoder_h.view(-1, self.h_dim), pool_h], dim=1)
                decoder_h = self.mlp(decoder_h)
                decoder_h = torch.unsqueeze(decoder_h, 0)  # (1, batch, h_dim)
                state_tuple = (decoder_h, state_tuple[1])

            embedding_input = rel_pos
            decoder_input = self.spatial_embedding(embedding_input)  # (batch, embedding_dim)
            decoder_input = decoder_input.view(1, batch, self.embedding_dim)  # (1, batch, embedding_dim)

            pred_traj_fake_rel.append(rel_pos.view(batch, -1))
            last_pos = curr_pos

        pred_traj_fake_rel = torch.stack(pred_traj_fake_rel, dim=0)  # (seq_len, batch, 2)
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end,  vx, vy, ax, ay, agent_type, size, traffic_state, user_noise=None,):
        ...
        if self.pooling_type:
            ...
            mlp_decoder_context_input = torch.cat(
                [combined_encoding.view(-1, self.encoder_h_dim*3), pool_h], dim=1)
