...
import numpy as np
import os
from nova.behavior_net import EncoderRNN, TypeEncoder, Behavior_Latent_Decoder
...
class Behavior_policy:
    def __init__(self, args, logger):
        ...
        # include both RNN and Type encoder outputs
    ...
    def init_behavior_net(self):
        self.behavior_encoder_rnn = []
        self.behavior_encoder_type = []
        self.behavior_decoder = []
        self.behavior_optimizer = []

        for i in range(self.n_agents):
            self.behavior_encoder_rnn.append(EncoderRNN(input_size=self.args.obs_shape_single,
            ...
            self.behavior_encoder_type.append(TypeEncoder(agent_type_dim=3,
                                                          embedding_dim=self.args.embedding_dim,
                                                          h_dim=self.args.h_dim).to(self.device))
            ...
            self.encoder_rnn_parameters = list(self.behavior_encoder_rnn[i].parameters())
            self.encoder_type_parameters = list(self.behavior_encoder_type[i].parameters())
            self.decoder_parameters = list(self.behavior_decoder[i].parameters())

            self.behavior_optimizer.append(
                torch.optim.Adam(self.encoder_rnn_parameters + self.encoder_type_parameters + self.decoder_parameters,
                                lr=self.args.lr_behavior,
                                eps=self.optim_eps,
                                weight_decay=self.weight_decay))

    # Update the behavior latent in the rollout
    def latent_update(self, history, agent_types, encoder_hidden, prev_latent):
        ...
        for i in range(self.n_agents):
            ...
            agent_type_per = agent_types[:, i, :].reshape(-1, 1)  # TODO: agent type shape
            agent_type_per = torch.Tensor(agent_type_per).to(self.device)

            out, encoder_hidden_per, latent_rnn = self.behavior_encoder_rnn[i](history_per, encoder_hidden_per)
            latent_type = self.behavior_encoder_type[i](agent_type_per)
            
            # Combine latent representations from RNN and TypeEncoder
            latent = torch.cat([latent_rnn, latent_type], dim=-1)
    ...
    def learn(self, batch, t_env):
        train_info = {
            'behavior_loss': 0.,
            "behavior_encoder_rnn_grad_norm": 0.,
            "behavior_encoder_type_grad_norm": 0.,
        ...
        agent_types = batch["agent_types"][:, :-1]  # get agent types
        ...
        for i in range(self.n_agents):
            ...
            for j in range(num_history - 1):
                ...
                curr_agent_type = torch.Tensor(agent_types[:, j*self.max_history_len, i, :]).to(self.device)
                # Encoder
                curr_history_en = curr_history.reshape(n_thread * max_vehicle_num, self.max_history_len, obs_dim)
                _, encoder_hidden, latent_rnn = self.behavior_encoder_rnn[i](curr_history_en, encoder_hidden)
                latent_type = self.behavior_encoder_type[i](curr_agent_type)
            ...
            if self._use_max_grad_norm:
                behavior_encoder_rnn_grad_norm = nn.utils.clip_grad_norm_(self.behavior_encoder_rnn[i].parameters(), self.max_grad_norm)
                behavior_encoder_type_grad_norm = nn.utils.clip_grad_norm_(self.behavior_encoder_type[i].parameters(), self.max_grad_norm)
                behavior_decoder_grad_norm = nn.utils.clip_grad_norm_(self.behavior_decoder[i].parameters(), self.max_grad_norm)
            else:
                behavior_encoder_rnn_grad_norm = get_grad_norm(self.behavior_encoder_rnn[i].parameters())
                behavior_encoder_type_grad_norm = get_grad_norm(self.behavior_encoder_type[i].parameters())
            ...
            train_info['behavior_encoder_rnn_grad_norm'] += behavior_encoder_rnn_grad_norm.item()
            train_info['behavior_encoder_type_grad_norm'] += behavior_encoder_type_grad_norm.item()
    ...
    def save_models(self, path):
        for i, behavior_encoder_rnn in enumerate(self.behavior_encoder_rnn):
            torch.save(behavior_encoder_rnn.state_dict(), f"{path}/behavior_encoder_rnn_{i}.th")
        for i, behavior_encoder_type in enumerate(self.behavior_encoder_type):
            torch.save(behavior_encoder_type.state_dict(), f"{path}/behavior_encoder_type_{i}.th")
    ...
    def load_models(self, paths:list, load_optimisers=False):
        ...
        for i, behavior_encoder_rnn in enumerate(self.behavior_encoder_rnn):
            behavior_encoder_rnn.load_state_dict(
                torch.load("{}/behavior_encoder_rnn_{}.th".format(paths[i], i),
                            map_location=lambda storage, loc: storage))
        for i, behavior_encoder_type in enumerate(self.behavior_encoder_type):
            behavior_encoder_type.load_state_dict(
                torch.load("{}/behavior_encoder_type_{}.th".format(paths[i], i),
                            map_location=lambda storage, loc: storage))
        for i, behavior_decoder in enumerate(self.behavior_decoder):
