...
import os
from nova.GAT_Net import PredictionEncoder
...
class Prediction_policy:
    def __init__(self, args, logger):
        ...
        if self.args.GAT_use_behavior:
            self.encoder_input_dim = self.args.obs_shape_single + self.args.latent_dim
        else:
            self.encoder_input_dim = self.args.obs_shape_single
    ...
    def init_GAT_net(self):
        ...
        for i in range(self.n_agents):
            self.pred_encoder.append(PredictionEncoder(input_size=self.encoder_input_dim, 
                                                       hidden_size=self.args.GAT_hidden_size,
                                                       output_size=self.args.attention_dim,
                                                       activation=self.args.activation,
                                                       batch_norm = self.args.batch_norm,
                                                       dropout=self.args.dropout,
                                                       GAT_use_behavior = self.args.GAT_use_behavior
                                                       ).to(self.device))
            ...
            self.encoder_parameters = list(self.pred_encoder[i].parameters())
    ...
    def GAT_latent_update(self, history_single, encoder_hidden, behavior_latent=None):
        ...
        _, _, _, attention_dim = encoder_hidden.shape  # attention_dim = 2 * agrs.attention_dim

        new_latent = []
        new_hidden = []

        for i in range(n_agents):
            history_single_per = torch.Tensor(history_single[:, i]).to(self.device)  # [n_thread, max_vehicle_num, obs_dim]
            if self.args.GAT_use_behavior:
                ...
                encoder_hidden_per = torch.Tensor(behavior_latent[:, i]).to(self.device)
            else:
                encoder_hidden_per = torch.zeros((n_thread, max_vehicle_num, latent_dim)).to(self.device)
            ...
            encoder_hidden_per = encoder_hidden_per.reshape(n_thread * max_vehicle_num, latent_dim)

            out_latent_per, encoder_hidden_per = self.pred_encoder[i](history_single_per, encoder_hidden_per)
            encoder_hidden_per = encoder_hidden_per.reshape(n_thread, max_vehicle_num, attention_dim)
            encoder_hidden_per = encoder_hidden_per.unsqueeze(1)
            new_hidden.append(encoder_hidden_per)
            new_latent.append(out_latent_per.reshape(n_thread, max_vehicle_num, -1).unsqueeze(1))

        attention_latent = torch.cat(new_latent, dim=1)  # [n_thread, n_agent, max_vehicle_num, bottleneck_dim(attention_dim)]
        attention_latent = attention_latent.cpu().detach().numpy()
        # Output: Attention latent: [n_thread, n_agent, max_vehicle_num, attention_dim]
        encoder_hidden = torch.cat(new_hidden, dim=1)
        encoder_hidden = encoder_hidden.cpu().detach().numpy()
        
        return attention_latent, encoder_hidden

    def prediction_batch_wrapper(self, history, encoder_hidden, mask, behavior_latent=None):
        """
        ...
        input_hidden = torch.zeros((self.prediction_batch_size, max_vehicle_num, 1, attention_dim))
        ...
        for i in range(self.prediction_batch_size):
            ...
            input_hidden[i, :, :, :] = encoder_hidden[batch_idx, time_idx, :, :].reshape(max_vehicle_num, 1, attention_dim)
        ...
        return input_traj, input_hidden, input_latent, actual_traj, mask_over_traj
    ...
    def learn(self, batch, t_env):
        ...
        att_encoder_hidden = batch["att_encoder_hidden"][:, :-1]
        ...
        for i in range(n_agent):
            ...
            agent_encoder_hidden = att_encoder_hidden[:, :, i]
            ...
            input_traj, input_hidden, input_latent, actual_traj, mask_over_traj = \
                self.prediction_batch_wrapper(agent_history, agent_encoder_hidden, mask, agent_behavior_latent)

            # Encoder (GAT)
            history_single_per = input_traj.reshape(self.args.pred_batch_size, self.max_vehicle_num, obs_dim)
            encoder_hidden = input_hidden.reshape(self.args.pred_batch_size * self.max_vehicle_num,
                                                    2 * self.args.attention_dim)
            ...
            # Output (Attention after encoding): [prediction_batch * max_vehicle_num, attention_dim * 2]
            attention_latent, encoder_hidden = self.pred_encoder[i](history_single_per, encoder_hidden)
            ...
            pred_traj = self.pred_decoder[i](input_traj, actual_traj, attention_latent)
            ...
            if self._use_max_grad_norm:
                pred_encoder_grad_norm = nn.utils.clip_grad_norm_(self.pred_encoder[i].parameters(), self.max_grad_norm)
            else:
                pred_encoder_grad_norm = get_grad_norm(self.pred_encoder[i].parameters())
    ...
    def save_models(self, path):
        for i, pred_encoder in enumerate(self.pred_encoder):
            torch.save(pred_encoder.state_dict(), f"{path}/pred_encoder_{i}.th")
    ...
    def load_models(self, paths:list, load_optimisers=False):
        ...
        for i, pred_encoder in enumerate(self.pred_encoder):
            pred_encoder.load_state_dict(
                torch.load("{}/pred_encoder_{}.th".format(paths[i], i),
