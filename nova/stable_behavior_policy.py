...
import numpy as np
import os
from nova.behavior_net import Behavior_Encoder, Behavior_Latent_Decoder
...
class Behavior_policy:
    ...
    def init_behavior_net(self):
        ...
        for i in range(self.n_agents):
            self.behavior_encoder.append(Behavior_Encoder(input_size=self.args.obs_shape_single,
                                                          hidden_size=self.args.behavior_encoder_dim,
    ...
    def latent_update(self, history, encoder_hidden, prev_latent):
        ...
        # RNN (initial): [n_thread, num_layers, n_agent, max_vehicle_num, encoder_dim]
        # RNN (per agent): [num_layers, n_thread * max_vehicle_num, encoder_dim]
        ...
        _, num_layers, _, _, encoder_dim = encoder_hidden.shape

        for i in range(self.n_agents):
            encoder_hidden_per = torch.Tensor(encoder_hidden[:, :, i]).permute((1, 0, 2, 3)).to(self.device)
            encoder_hidden_per = encoder_hidden_per.reshape(num_layers, n_thread * max_vehicle_num, encoder_dim)

            history_per = history[:, i, :, :, :].reshape(-1, max_history_len, obs_dim)
            history_per = torch.Tensor(history_per).to(self.device)

            encoder_hidden_per, latent = self.behavior_encoder[i](history_per, encoder_hidden_per)
            new_latent.append(latent.reshape(n_thread, max_vehicle_num, -1).unsqueeze(1))  # adding agent dim; [n_thread, 1, max_vehicle_num, output_size]

            encoder_hidden_per = encoder_hidden_per.reshape(num_layers, n_thread, max_vehicle_num, encoder_dim)
            encoder_hidden_new.append(encoder_hidden_per.permute((1, 0, 2, 3)).unsqueeze(2))  # adding agent dim; [n_thread, num_layers, 1, max_vehicle_num, encoder_dim]

        new_latent = torch.cat(new_latent, dim=1)  # [n_thread, n_agent, max_vehicle_num, output_size]
        new_latent = new_latent.cpu().detach().numpy()

        # Soft update
        new_latent = (1 - self.soft_update_coef) * prev_latent + new_latent * self.soft_update_coef

        encoder_hidden = torch.cat(encoder_hidden_new, dim=2)  # [n_thread, num_layers, n_agent, max_vehicle_num, encoder_dim]
    ...
    def learn(self, batch, t_env):
        ...
        for i in range(self.n_agents):
            ...
            encoder_hidden = torch.zeros((self.args.num_encoder_layer, n_thread * self.max_vehicle_num,
                                          self.args.behavior_encoder_dim * 2)).to(self.device)
            ...
            for j in range(max_episode_len - 1 - self.max_history_len):
                ...
                encoder_hidden, new_latent = self.behavior_encoder[i](curr_history_en, encoder_hidden)
