...
import os
from nova.prediction_encoder import PredictionEncoder
...
class Prediction_policy:
    ...
    def init_GAT_net(self):
        ...
        for i in range(self.n_agents):
            ...
            #######################################
            self.pred_decoder.append(Prediction_Decoder(input_size=self.args.obs_shape_single,
            ...
            self.pred_optimizer.append(
                torch.optim.Adam(self.encoder_parameters + self.decoder_parameters,
                                 lr=self.args.lr_predict,
                                 eps=self.optim_eps,
                                 weight_decay=self.weight_decay))
 
    # GAT update for rollout
    # Note: GAT always has only one layer
    def GAT_latent_update(self, history_single, behavior_latent=None):
        ...
        for i in range(n_agents):
            ...
            out_latent_per = self.pred_encoder[i](history_single_per, encoder_hidden_per)
    ...
    def prediction_batch_wrapper(self, history, mask, behavior_latent=None):
        """
        ...
        return input_traj, input_latent, actual_traj, mask_over_traj
    ...
    def learn(self, batch, t_env):
        ...
        for i in range(n_agent):
            ...
            input_traj, input_latent, actual_traj, mask_over_traj = \
                self.prediction_batch_wrapper(agent_history, mask, agent_behavior_latent)
            ...
            if self.args.GAT_use_behavior:
                ...
                latent_per = torch.zeros(self.args.pred_batch_size, self.max_vehicle_num, latent_dim).to(self.device)

            # Output (Attention after encoding): [prediction_batch * max_vehicle_num, attention_dim * 2]
            attention_latent = self.pred_encoder[i](history_single_per, latent_per)
